...
# Код ищет файл logo.png в корне вашего репозитория
...
if st.toggle("Есть собственная сетевая инфраструктура?"):
    ...
    with c_net2:
        ...
        if data['2.4. NGFW (Вендор)']: score += 20 # Баллы за NGFW
...
# --- БЛОК 3: ИНФОРМАЦИОННАЯ БЕЗОПАСНОСТЬ (СКОРИНГ) ---
...
if st.toggle("Есть внешние веб-ресурсы?"):
    ...
    data['4.3. БД'] = get_multi("4.3. СУБД:", ["PostgreSQL", "MySQL", "Oracle", "MS SQL"], "db")
...
if st.button("🚀 Сформировать экспертную аналитику"):
    if not data:
        st.error("Сначала заполните данные!")
    else:
        final_excel = create_report(data, min(score, 100))
        st.success(f"Анализ завершен. Индекс зрелости: {min(score, 100)}/100")
        st.download_button("📥 Скачать экспертный Excel", final_excel, "Audit_Expert_Report.xlsx")

st.info("Разработано Ivan Rudoy. По вопросам автоматизации звоните!")
